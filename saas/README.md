...
- **FAQ + consultation sections**: the capture also recorded a real "Câu hỏi thường gặp" (FAQ) section — the exact real question "Khi nào cần chuyển đổi hội thoại từ Chatbot sang nhân viên hỗ trợ trực tiếp?" is reproduced verbatim, answered with this product's own real, verified behavior (not preny.ai's real, inaccessible internal process) — and a "Nhận tư vấn từ chuyên gia AI" consultation CTA section, both now on `Home.jsx`. A third real section, "Hợp tác cùng các đơn vị báo chí uy tín" (backed by real thanhnien.vn/24h.com.vn/cafef.vn/soha.vn article links), is deliberately **not** reproduced — those links are real press coverage of preny.ai specifically, and reusing them here would misattribute that coverage to a product that never received it.
- **Homepage embeds its own live demo widget**, mirroring the real capture's observed `bot-embed.js` script on preny.ai's own homepage. Backed by a real (not fabricated) demo Account/Chatbot this platform's own operators run (`prisma/seed.js`'s `DEMO_CHATBOT_ID`, exposed publicly by `GET /api/demo/widget-key`) — same real Claude behavior and conversation-quota enforcement as any other tenant's chatbot, just self-operated. Verified live end-to-end (real widgetKey served, real conversation created against it).
...
- **Per-route CORS, not one blanket policy**: the dashboard/auth/billing/admin surface is locked to a single configured origin with credentials; the embeddable widget's public endpoints (`/api/widget/*`, `/api/uploads/widget`) separately allow any origin, since embedding on arbitrary third-party sites is the entire point of a chat widget — see `app.js`. Regression-tested (`app.cors.test.js`).
- **Rate limiting** on `/api/auth/login` (10/15min) and `/api/auth/register` (10/hour) per IP — real brute-force/credential-stuffing protection, not just documentation. Regression-tested (`auth.ratelimit.test.js`).
...
- **No real SMTP account is configured.** `/api/auth/forgot-password` always returns the same generic response (`{"message": "Nếu email tồn tại..."}`) regardless of whether the email exists or SMTP is configured — that's deliberate, not a limitation (a different response in either case would let an anonymous caller enumerate accounts or probe this deployment's mail config). What differs without real `SMTP_HOST`/`SMTP_USER`/`SMTP_PASSWORD` (see `.env.example`) is that the reset link is only logged server-side (`email.service.js`) instead of actually emailed — the real `PasswordResetToken` row, 1-hour expiry, single-use enforcement, and full-session revocation on reset all still work exactly as they would with real SMTP; only the delivery channel is missing.

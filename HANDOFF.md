...
- **`capture/scroll.js`** + **`capture/interact.js`** (mới): cuộn hết trang + tự động mở mọi accordion/tab/dropdown/menu/popup thật (`aria-expanded="false"`/`role=tab`/`summary`, click đúng 1 lần mỗi phần tử, lặp nhiều vòng để bắt cả nội dung mount lồng nhau) trước khi chụp bất kỳ artifact nào — để asset lazy-load + nội dung/asset chỉ lộ ra sau tương tác đều vào HAR thật. Chỉ có tác dụng cho lần capture MỚI (không hồi tố được dữ liệu đã có trong repo). `interact.js` đã verify cơ chế (mở nested trigger đúng ở vòng sau) bằng fixture cục bộ, nhưng **chưa verify được với `preny.ai` thật hay bất kỳ site sống nào** — sandbox phiên này chặn egress ra TOÀN BỘ domain ngoài allowlist (đã verify lại bằng `curl` tới `preny.ai`/`example.com`/`playwright.dev` đều 403 + proxy status endpoint) — đây là blocker hạ tầng, không phải blocker sửa được bằng mã nguồn.
- Lần chạy QA gần nhất (sau khi thêm asset localization + no-origin-dependency gate, verify lại từ trạng thái sạch hoàn toàn): `passed: true`, `13/13` output check, `0` analysis issue, `0` origin-dependency issue, `rebuildBuild.passed: true`. Coverage asset thật: `64/221` referenced localized từ HAR hiện có (157 còn thiếu vì chưa từng được request lúc capture — xem `capture/scroll.js` ở trên). Verify thêm bằng Playwright thật chạy `vite preview`: title/nav/hero/accordion đúng thật, accordion trigger toggle `aria-expanded` đúng khi click, 0/36 network request trỏ `preny.ai`, 0 console/page error.
...
| `capture/interact.js` | Playwright `page` object, gọi từ `capture/index.js` (sau `scrollFullPage()` lần 1, trước lần 2) |
...
29. **Sửa bug tự phát hiện trong `generator/harAssets.js`**: entry HAR mimeType `text/html` (chính trang đã capture) từng bị localize nhầm thành asset nhị phân — link nội bộ `href="/"` (logo, "Trang chủ") bị trỏ vào 1 bản copy 293KB của toàn trang thay vì giữ nguyên `/`. Thêm `isDocumentResponse()` loại trừ `text/html` khỏi đường localize nhị phân, cùng cơ chế với `text/css` (đã có sẵn `localizeStylesheet` riêng). Verify lại: hết file `.bin`, hết bug, QA vẫn pass, coverage báo cáo đúng lại `64/221` (4 link tự trỏ `/` không còn bị đếm nhầm là "đã localize").
30. **Hoàn thiện capture để bao phủ tối đa dữ liệu thật — thêm `capture/interact.js`.** Yêu cầu: tự động mở tất cả accordion/tab/dropdown/menu/popup, thu thập asset+request phát sinh sau tương tác. `openAllInteractive(page)`: quét lặp (tối đa 6 vòng) các phần tử `[aria-expanded="false"]`, `[role="tab"]:not([aria-selected="true"])`, `summary` — chọn có chủ đích các selector ARIA/semantic chuẩn (không hardcode theo preny.ai), click từng phần tử ĐÚNG 1 LẦN (đánh dấu `data-capture-opened` ngay trước khi click — đây là cơ chế đảm bảo vòng lặp luôn dừng, không bao giờ double-toggle 1 phần tử về lại trạng thái đóng). Chạy trên trang SỐNG (còn đầy đủ JS/handler thật của framework gốc) nên click thật sự trigger đúng hành vi thật — không phải giả lập. Nội dung mới lộ ra (kể cả Radix portal chỉ mount sau khi phần tử cha mở) được vòng lặp SAU bắt lại đúng — đã verify cơ chế này bằng fixture cục bộ (`page.setContent()`, không cần mạng): dựng 1 trigger lồng bên trong panel accordion, chỉ tồn tại trong DOM sau khi accordion mở — verify vòng 2 phát hiện và click đúng nó. Nối vào `capture/index.js`: `scrollFullPage()` → `openAllInteractive()` → `scrollFullPage()` lần 2 (bắt lazy-image mới lộ ra trong panel vừa mở) → mới lưu artifact. HAR (`mode:'full'`, đã có) tự động ghi lại MỌI request trong suốt vòng đời context, nên request phát sinh từ tương tác tự động vào HAR, không cần thêm cơ chế riêng. **KHÔNG verify được với `preny.ai` thật hay bất kỳ site sống nào trong phiên này** — xác nhận lại bằng chứng: `curl` tới `preny.ai`, `example.com`, `playwright.dev` đều trả `CONNECT tunnel failed, response 403`, và endpoint trạng thái proxy (`recentRelayFailures`) xác nhận `connect_rejected`/`policy denial` cho cả 3 domain — đây là chính sách egress chặn TOÀN BỘ domain ngoài allowlist của MÔI TRƯỜNG SANDBOX này, không phải riêng `preny.ai`, và không thể giải quyết bằng sửa mã nguồn trong repo. Pipeline `--skip-capture` (dữ liệu HAR hiện có) không bị ảnh hưởng bởi thay đổi này — verify lại toàn bộ theo mục 19, vẫn `passed:true`, `13/13`, `0` origin-dependency issue.
...
| 9 | Coverage asset thật hiện tại chỉ `64/221` (157 asset chưa từng được browser request lúc capture — chủ yếu ảnh `loading="lazy"` dưới màn hình đầu + nội dung sau tương tác, vì lần capture đã lưu trong repo KHÔNG cuộn trang, KHÔNG mở widget) | Đã biết, có bằng chứng (`public/assets/manifest.json`, `qa/output/report.json.assetManifest`) | **Mã nguồn đã đủ** — `capture/scroll.js` + `capture/interact.js` (mới, phiên này) khắc phục tận gốc cho lần capture MỚI. Không thể hồi tố dữ liệu HAR đã có sẵn, và không thể verify 2 module này với `preny.ai` thật trong phiên này vì **egress toàn bộ domain ngoài allowlist bị chặn ở tầng sandbox** (verify bằng `curl` tới `preny.ai`/`example.com`/`playwright.dev` đều 403 + endpoint trạng thái proxy) — đây là blocker hạ tầng, không phải blocker có thể giải quyết bằng mã nguồn trong repo. Cần chạy `npm run capture` thật ở môi trường có mạng để đóng hoàn toàn khoảng trống này và verify `capture/interact.js` lần đầu trên DOM thật |
| 10 | 2 widget Radix (combobox chọn ngôn ngữ, dialog menu mobile) không khôi phục được nội dung popup — trigger có `aria-expanded`/`aria-controls` thật nhưng target ID không tồn tại trong DOM đã capture (dữ liệu hiện có trong repo) | Đã biết, có bằng chứng (đã verify bằng script kiểm tra `aria-controls` id tồn tại trong `dom.html`) | `capture/interact.js` (mới, phiên này) đã thêm cơ chế click mở đúng 2 widget này ở lần capture kế tiếp (chọn theo `aria-expanded="false"`, không phân biệt loại widget) — cùng blocker hạ tầng như #9 khiến chưa verify được với dữ liệu thật trong phiên này |

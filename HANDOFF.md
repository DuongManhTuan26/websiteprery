...
29. **Sửa bug tự phát hiện trong `generator/harAssets.js`**: entry HAR mimeType `text/html` (chính trang đã capture) từng bị localize nhầm thành asset nhị phân — link nội bộ `href="/"` (logo, "Trang chủ") bị trỏ vào 1 bản copy 293KB của toàn trang thay vì giữ nguyên `/`. Thêm `isDocumentResponse()` loại trừ `text/html` khỏi đường localize nhị phân, cùng cơ chế với `text/css` (đã có sẵn `localizeStylesheet` riêng). Verify lại: hết file `.bin`, hết bug, QA vẫn pass, coverage báo cáo đúng lại `76/221` (4 link tự trỏ `/` không còn bị đếm nhầm là "đã localize").
